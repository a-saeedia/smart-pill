...
const HARD_HINTS: ReadonlyArray<{ label: string; re: RegExp }> = [
  {
    ...
    re: /\b(design|architect(?:ure)?|trade-?offs?|schema|api contract)\b/i,
  },
  {
    label: 'debug/trace',
    re: /\b(debug|trace|diagnos\w*|root cause|repro(?:duce)?|stack ?trace|regression)\b/i,
  },
  {
    label: 'concurrency/systems',
    re: /\b(concurren(?:cy|t)|deadlock|race condition|distributed|throughput|latency|backpressure)\b/i,
  },
  {
    label: 'security/crypto',
    re: /\b(security|vulnerab\w*|crypto|auth(?:entication|orization)?|owasp|exploit|injection|xss|csrf|secret)\b/i,
  },
  {
    label: 'infra/deploy',
    re: /\b(docker|kubernetes|k8s|terraform|ci\/?cd|pipeline|infra(?:structure)?|deploy\w*|migration)\b/i,
  },
  {
    label: 'refactor/optimize',
    re: /\b(refactor\w*|optimi[sz]\w*|performance|profil(?:e|ing)|benchmark\w*|scal(?:e|ing|ability))\b/i,
  },
];

const FAILURE_HINTS =
  /\b(fix|broken|fails?|failing|error|crash(?:es|ed)?|segfault|cannot|can't|unable|throws?|exception)\b/i;
