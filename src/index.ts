    ...
    'pill_digest',
    {
      ...
      inputSchema: {
        ...
        focus: z
          ...
          .optional()
          .describe(
            'Bias the extractive digest toward lines containing this term (e.g. the task about to start)',
          ),
      },
    },
    async ({ text, maxChars, task, focus }) => {
      ...
      if (task === 'summarize' && config.openRouterKey) {
        ...
        if (res.ok) {
          ...
          return {
            content: [{ type: 'text' as const, text: `SUMMARY (${res.model})\n\n${res.content}` }],
          };
        }
  ...
  server.registerTool(
    ...
    async ({ entries }) => {
      ...
      if (truncatedValues > 0)
        warnings.push(`${truncatedValues} value(s) truncated to ${MAX_VALUE} chars`);
    ...
    async ({ topic, limit }) => {
      ...
      const text =
        hits.length === 0
          ...
          : [
              `${hits.length} hit(s) for "${topic}":`,
              ...hits.map((h) => `- ${h.key}: ${h.value}`),
            ].join('\n');
    ...
    'pill_context',
    {
      ...
      inputSchema: {
        ...
        limit: z
          ...
          .max(50)
          .optional()
          .describe('Max memory hits (default 8)'),
      },
    },
    async ({ topic, limit }) => {
      const data = await store.read();
      ...
      if (!verdict.escalate && !force) {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Skipped escalation (score ${verdict.score} < ${verdict.threshold}). Use force:true to override.`,
            },
          ],
        };
      }
            ...
            : [
                {
                  ...
                  content:
                    'You are a senior staff engineer. Answer directly, terse, with exact commands and real verification.',
                },
              ]),
